function Invoke-HelpspotApi {
    ...
    begin {
        ...
        if ($Params) {
            ...
            foreach ($object in $Params.GetEnumerator()) {
                ...
                if ($config.Parameters[$object.Name]) {
                    # add it to string
                    $ExtraParamString.Add("$($object.Name)", "$($object.Value)")
                    # remove from hashtable for future check if all required params are given.
                    $config.Parameters.remove($object.Name)
                }
                else {
                    Write-Warning "$($object.Name) is not a valid parameter for this endpoint only: `n$((Get-HelpspotApiDef -Apidefinition $ApiEndpoint).Parameters.keys -join ', ')"
                }
            }
    ...
    process {
        ...
        try {
            ...
            $RequiredParameters = $config.Parameters.GetEnumerator() | Where-Object { $_.Value.Required -eq $true }
            if (-Not $RequiredParameters) {
                Invoke-RestMethod -Headers $headers -Uri $urlBuilder -Method $config.Method
            }
            else {
